...
for i in range(ncl):
    ave[i] = i ** indx   # indx = 0 makes the distribution uniform
...
parser = argparse.ArgumentParser(description='PyTorch Implementation of LCT for CIFAR10/100')
...
parser.add_argument('--epochs', default=1600, type=int, help='number of epochs to train')
...
parser.add_argument('--type', default='100', type=int, help='cifar10 or 100')
...
if args.type == 10:
    print('CIFAR-10 laoding...')
    trainset = CIFAR10Instance(root=args.datadir, train=True, download=True, transform=transform_train)
    ...
    testset = CIFAR10Instance(root=args.datadir, train=False, download=True, transform=transform_test)
    testloader = torch.utils.data.DataLoader(testset, batch_size=args.bs, shuffle=False, num_workers=16)
else:
    print('CIFAR-100 laoding...')
    trainset = CIFAR100Instance(root=args.datadir, train=True, download=True, transform=transform_train)
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.bs, shuffle=True, num_workers=16)
    trainloader_ = torch.utils.data.DataLoader(trainset, batch_size=args.bs, shuffle=False, num_workers=16)
    trainloader_1 = torch.utils.data.DataLoader(trainset, batch_size=args.bs * 2, shuffle=True, num_workers=16)
    trainloader_2 = torch.utils.data.DataLoader(trainset, batch_size=args.bs * 4, shuffle=True, num_workers=16)

    testset = CIFAR100Instance(root=args.datadir, train=False, download=True, transform=transform_test)
    testloader = torch.utils.data.DataLoader(testset, batch_size=args.bs, shuffle=False, num_workers=16)
...
for epoch in range(args.epochs):
    ...
    PATH = '/raid/lql/models_saved/cifar100.pth'
